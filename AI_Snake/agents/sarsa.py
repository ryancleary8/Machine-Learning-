# agents/agent_sarsa.py - SARSA Agent (on-policy)
...
class SARSAAgent:
    """SARSA agent using tabular Q-table (on-policy learning)"""
    ...
    def update(self, state, action, reward, next_state, next_action, done):
        """SARSA update rule (uses next action actually taken)"""
        current_q = self._get_q_value(state, action)
        
        if done:
            target_q = reward
        else:
            # SARSA uses the next action that will actually be taken (on-policy)
